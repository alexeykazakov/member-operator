...
func TestOverallStatusCondition(t *testing.T) {
	...
	t.Run("All components ready", func(t *testing.T) {
		...
		getHostClusterFunc := newGetHostClusterReady
		reconciler, req, fakeClient := prepareReconcile(t, requestName, getHostClusterFunc, allNamespacesCl, append(nodeAndMetrics, memberOperatorDeployment, memberStatus, prodConfig, githubSecret)...)
	...
	t.Run("member operator deployment revision check", func(t *testing.T) {
		...
		t.Run("deployment version check is disabled", func(t *testing.T) {
			...
			t.Run("when environment is prod but github secret is not present", func(t *testing.T) {
				...
				reconciler, req, fakeClient := prepareReconcile(t, requestName, getHostClusterFunc, allNamespacesCl, append(nodeAndMetrics, memberOperatorDeployment, memberStatus, prodConfig)...) // we don't pass the github secret object
...
func prepareReconcile(t *testing.T, requestName string, getHostClusterFunc func(fakeClient client.Client) cluster.GetHostClusterFunc, allNamespacesClient *test.FakeClient, initObjs ...client.Object) (*Reconciler, reconcile.Request, *test.FakeClient) {
